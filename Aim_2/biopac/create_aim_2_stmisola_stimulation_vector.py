...
def get_parser():
    ...
    parser.add_argument('-stim_amp', default=0.2, required=False, type=float,
...
def main():
    ...
    if args.stim_amp > 10.0 or args.stim_amp <= 0:
    ...
    try:
       ...
       filename = args.type + '_f_' + \
          ...
          str(args.stim_amp)  + 'ma_samp_f_' + \
          str(args.samp_f)  + 'hz'
    print(filename)

    #Create random generator setting seed equal to amplitude*100
    rng = np.random.default_rng(seed=int(args.stim_amp*100))

    #Create no_stim_duration list and then shuffle
    no_stim_durations=args.no_stim_durations.split(',')
    rng.shuffle(no_stim_durations)

    #Convert to integer and then sum
    ...
    print('length in seconds of stim vector')
    print(str(int(stim_vector.shape[0])/5000))
    ...
    for block in np.arange(0, args.n_stim_blocks):
        ...
        if args.type.lower() == 'biphasic':
            ...
            stim_block = args.stim_amp*np.tile(single_stim_block, args.stim_duration*args.stim_f)
    ...
    stim_vector[stim_vector > args.stim_amp] = 0
