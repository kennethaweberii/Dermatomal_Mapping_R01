...
rm -f subject_metrics_rois_perRun.txt
rm -f subject_metrics_rois_perRun_voxels.txt
echo task run cope subject region zscore_sc >> subject_metrics_rois_perRun.txt
echo task run cope subject region voxels_sc >> subject_metrics_rois_perRun_voxels.txt
        ...
        for roi in ${rois[@]}; do
            for run in ${runs[@]}; do
               ...
               session=""
                ...
                zscore=`fslstats ${data_path}/${subject}/ses-${session}spinalcord/func/run-${run}/${subject}_ses-${session}spinalcord_task-${task}_run-${run}_bold_mc2_pnm_stc2template_smooth225_trialwise_second_level.gfeat/cope1.feat/thresh_zstat1.nii.gz -k ${data_path}/masks/rois_n40/${roi}.nii.gz -M`
                voxels=`fslstats ${data_path}/${subject}/ses-${session}spinalcord/func/run-${run}/${subject}_ses-${session}spinalcord_task-${task}_run-${run}_bold_mc2_pnm_stc2template_smooth225_trialwise_second_level.gfeat/cope1.feat/thresh_zstat1.nii.gz -k ${data_path}/masks/rois_n40/${roi}.nii.gz -V | cut -d " " -f1`
                ...
                echo ${task} run-${run} cope${cope} ${subject} ${roi} ${voxels} >> subject_metrics_rois_perRun_voxels.txt
