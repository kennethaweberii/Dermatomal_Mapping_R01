            ...
            for measure in measures:
                ...
                if measure == 'zscore_sc':
                    ...
                    if region in ['left_sc_gm_mask', 'right_sc_gm_mask', "left_sc_mask", 'right_sc_mask']:
                        ylim = [0, 2000]
                        ytickmarks = [0, 200, 400, 600, 800, 1000, 1200, 1400, 1600, 1800, 2000]
                    else:
                        ylim = [0, 600]
                ...
                sds = []
                for run in ['run-1', 'run-2', 'run-3']:
                    vals = dataset_roi[(dataset_roi['run'] == run) & (dataset_roi['region'] == region)& (dataset_roi['cope']== cope)][measure]
                ...
                x_all = dataset_roi[(dataset_roi['region'] == region)& (dataset_roi['cope']== cope)]['run'].map({'run-1': 1, 'run-2': 2, 'run-3': 3}).values
                y_all = dataset_roi[(dataset_roi['region'] == region)& (dataset_roi['cope']== cope)][measure].values
                slope, intercept, r_value, p_value, std_err = linregress(x_all, y_all)
                ...
                for i in range(n_runs):
                    for j in range(i + 1, n_runs):
                        ...
                        run_j = run_labels[j]
                        merged = pd.merge(
                            dataset_roi[(dataset_roi['run'] == run_i) & (dataset_roi['region'] == region)& (dataset_roi['cope']== cope)][['subject', measure]],
                            dataset_roi[(dataset_roi['run'] == run_j) & (dataset_roi['region'] == region)& (dataset_roi['cope']== cope)][['subject', measure]],
                            on='subject',
                            suffixes=('_' + run_i, '_' + run_j)
                        )
                        if not merged.empty:
                            stat, pval = ttest_rel(merged[measure + '_' + run_i], merged[measure + '_' + run_j])
                            result = f"Paired t-test {run_i} vs {run_j}: t={stat:.4f}, p={pval:.4g}, n={len(merged)}"
                        else:
                            result = f"No paired data for {run_i} vs {run_j}"
                        outname = f"{measure}_{region}_subject_{run_i}_{run_j}.txt"
                        pvals.append(pval)
