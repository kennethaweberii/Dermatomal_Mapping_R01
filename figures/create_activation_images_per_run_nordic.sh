...
output_path=/home/sbedard/Projects/Dermatomal_Mapping_R01/data/BIDS/derivatives/lindsay/BIDS/derivatives/results/figures_trialwise_run
...
for run in ${runs[@]}; do
	for cope in ${copes[@]}; do
		...
		overlay 0 1 PAM50_t2s_crop.nii.gz 300 800 ${data_path}/${run}/GroupLevel_trialwise_N40_${type}.gfeat/${cope}.feat/thresh_zstat1.nii.gz $min $max ${cope}_overlay
		overlay 0 1 PAM50_t2_crop.nii.gz 0 3000 ${data_path}/${run}/GroupLevel_trialwise_N40_${type}.gfeat/${cope}.feat/thresh_zstat1.nii.gz $min $max ${cope}_overlay_t2
